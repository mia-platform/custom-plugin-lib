...
function setBodyRelatedHeaders(body, options) {
  ...
  if (!body) {
    options.headers = { ...options.headers, 'content-length': '0' }
  }
}

function isStatusCodeNotAllowed(options, res) {
  return options.allowedStatusCodes && !options.allowedStatusCodes.includes(res.statusCode)
}
...
function makeCallAsStream(endUrl, method, body, options) {
  return new Promise(function promiseFunction(resolve, reject) {
    sget({
      ...
      if (isStatusCodeNotAllowed(options, res)) {
...
function makeCallAsBuffer(endUrl, method, body, options) {
  return new Promise(function promiseFunction(resolve, reject) {
    sget.concat({
      ...
      if (isStatusCodeNotAllowed(options, res)) {
        const errorMessage = res.headers['content-type'] === 'application/json' ? JSON.parse(data).message : ''

        return reject(statusCodeError(res.statusCode, options.allowedStatusCodes, errorMessage))
      }
      resolve(res)
    })
  })
...
function makeCall(hostname, path, method, querystring, body, options = {}) {
  ...
  const endUrl = new url.URL('http://example.com')
  endUrl.protocol = protocol
  endUrl.hostname = hostname
  endUrl.pathname = pathWithPrefix
  endUrl.port = port
  endUrl.search = searchParams.toString()
  ...
  if (asStream) {
    return makeCallAsStream(endUrl.href, method, convertBody(body), options)
  } else if (asJSON) {
    return makeCallAsJSON(endUrl.href, method, convertBody(body), options)
  } else if (asBuffer) {
    return makeCallAsBuffer(endUrl.href, method, convertBody(body), options)
  }
  return Promise.reject(new Error(`Unknown returnAs: ${options.returnAs}`))
}
