...
function statusCodeError(response, allowedStatusCodes) {
  const error = new Error(`Invalid status code: ${response.statusCode}. Allowed: ${allowedStatusCodes}.`)
  ...
  return new Promise(function promiseFunction(resolve, reject) {
    sget({
      ...
      if (options.allowedStatusCodes && !options.allowedStatusCodes.includes(res.statusCode)) {
        return reject(statusCodeError(res, options.allowedStatusCodes))
      }
      resolve(res)
    })
  })
