...
const httpOtherService = 'http://other-service'
const httpsOtherService = 'https://other-service'
...
module.exports = customService(async function clientGroups(service) {
  service.addRawCustomPlugin('POST', '/default', async function handler(req) {
    const { some } = req.body
    const proxy = req.getHttpClient(httpOtherService)
  ...
  service.addRawCustomPlugin('POST', '/custom', async function handler(req) {
    const { some } = req.body
    const proxy = req.getHttpClient(`${httpOtherService}:3000`)
    const res = await proxy.get('/res')
    const { id } = res.payload
    return { id, some }
  })
  ...
  service.addPreDecorator('/pre', async function handler(req) {
    ...
    const proxy = req.getHttpClient(`${httpsOtherService}:3000`)
    const res = await proxy.get('/res')
    const { id } = res.payload
